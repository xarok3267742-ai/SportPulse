...
class AnalysisImageProvider : ContentProvider() {
    override fun attachInfo(context: Context, info: ProviderInfo) {
        require(!info.exported) {
            "AnalysisImageProvider must not be exported"
        }
        require(info.grantUriPermissions) {
            "AnalysisImageProvider requires temporary URI grants"
        }
    ...
    private fun fileForUri(uri: Uri): File {
        ...
        if (
            ...
            throw FileNotFoundException("Invalid passport URI")
        }

        val fileName = uri.pathSegments.single()
        if (!FILE_NAME.matches(fileName)) {
            throw FileNotFoundException("Invalid passport file name")
        }

        val directory = File(providerContext.cacheDir, SHARE_DIRECTORY).canonicalFile
        val candidate = File(directory, fileName).canonicalFile
        if (candidate.parentFile != directory || !candidate.isFile) {
            throw FileNotFoundException("Passport image not found")
        }
        return candidate
    }

    companion object {
        internal const val SHARE_DIRECTORY = "analysis_passports"
        private const val MIME_TYPE = "image/png"
        private val FILE_NAME = Regex("[A-Za-z0-9_.-]+\\.png")

        fun uriFor(context: Context, file: File): Uri {
            val directory = File(context.cacheDir, SHARE_DIRECTORY).canonicalFile
            val candidate = file.canonicalFile
            require(candidate.parentFile == directory && candidate.isFile) {
                "Only generated passport images can be shared"
            }
            return Uri.Builder()
                .scheme("content")
        ...
        private fun authority(context: Context): String {
            return "${context.packageName}.analysis-images"
        }
    }
}
